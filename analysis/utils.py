...
def time_between_occurrences(data):
    time_diffs = {}
...
def avg_time_between_occurrences(time_diffs):
    avg_times = {num: diff.mean() for num, diff in time_diffs.items()}
    return avg_times


def count_number_pairs(data):
    pair_counts = {}
...
def most_least_common_pairs(pair_counts):
...
def boxplot_sum_by_decade(data):
...
def heatmap_number_pairs(pair_counts):
